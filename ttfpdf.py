...
# *******************************************************************************/
    ...
    def line(self, x1, y1, x2, y2):
        """Draw a line"""
        self._out(sprintf('%.2f %.2f m %.2f %.2f l S', x1 * self.k, (self.h - y1) * self.k, x2 * self.k,
                          (self.h - y2) * self.k))
    ...
    def text_with_direction(self, x, y, txt, direction='R'):
        ...
        if direction == 'R':
            s = sprintf('BT %.2f %.2f %.2f %.2f %.2f %.2f Tm (%s) Tj ET', 1, 0, 0, 1, x * self.k, (self.h - y) * self.k,
                        txt2)
        elif direction == 'L':
            s = sprintf('BT %.2f %.2f %.2f %.2f %.2f %.2f Tm (%s) Tj ET', -1, 0, 0, -1, x * self.k,
                        (self.h - y) * self.k, txt2)
        elif direction == 'U':
            s = sprintf('BT %.2f %.2f %.2f %.2f %.2f %.2f Tm (%s) Tj ET', 0, 1, -1, 0, x * self.k,
                        (self.h - y) * self.k, txt2)
        elif direction == 'D':
            s = sprintf('BT %.2f %.2f %.2f %.2f %.2f %.2f Tm (%s) Tj ET', 0, -1, 1, 0, x * self.k,
                        (self.h - y) * self.k, txt2)
    ...
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=False, link=''):
        ...
        if isinstance(border, basestring):
            ...
            if 'R' in border:
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', (x + w) * k, (self.h - y) * k, (x + w) * k,
                             (self.h - (y + h)) * k)
            if 'B' in border:
                s += sprintf('%.2f %.2f m %.2f %.2f l S ', x * k, (self.h - (y + h)) * k, (x + w) * k,
                             (self.h - (y + h)) * k)
        if txt != '':
            if align == 'R':
            ...
            if self.color_flag:
                s += ' Q'
            if link:
                self.link(self.x + dx, self.y + .5 * h - .5 * self.font_size, self.get_string_width(txt),
                          self.font_size, link)
        if s:
            self._out(s)
        self.lasth = h
    ...
    def image(self, file, x=None, y=None, w=0, h=0, type='', link='', is_mask=False, mask_img=0):
        ...
        if is_mask:
            x = (self.cur_page_size[0] if self.cur_orientation == 'P' else self.cur_page_size[1]) + 10
    ...
    def image_png_with_alpha(self, file, x, y, w, h, link):
        self.error('Unsupported Alpha Chanel PNG, Yet..')
        ...
        while i < l:
            ...
            if h <= 0x7F:
                ...
                if h <= 0xDF and i < l - 1:
                    ...
                    uni = (h & 0x0F) << 18 | (ord(s[i + 1]) & 0x3F) << 12 | (ord(s[i + 2]) & 0x3F) << 6 | (
                        ord(s[i + 3]) & 0x3F)
            if uni >= 0:
                add(uni)
            i += 1
    ...
    def coons_patch_mesh(self, x, y, w, h, col1=(), col2=(), col3=(), col4=(),
        ...
        if not isinstance(coords, dict):
            ...
            patch_array = [{
                               'f': 0,
                               'points': list(coords),
                               'colors': [{'r': col1[0], 'g': col1[1], 'b': col1[2]},
    ...
    def styled_ellipse(self, x0, y0, rx, ry=0, angle=0, a_start=0, a_finish=360, style='', line_style=None,
                       fill_color=None,
                       n_seg=8):
        if rx:
            if 'F' not in style and fill_color:
    ...
    def _parsegif(self, filename):
        ...
        try:
            ...
            if "transparency" in im.info:
                im.save(tmp, 'png', transparency=im.info['transparency'])
            else:
                im.save(tmp, 'png')
            info = self._parsepng(tmp)
            tmp.close()
        if info is None:
            self.error('Cannot decode gif image')
        return info
    ...
    def _parsepng(self, name):
        ...
        if isinstance(name, (cStringIO.InputType, cStringIO.OutputType)):
            f = name
            f.seek(0)
    ...
    def _putimage(self, info):
        if 'data' in info:
            ...
            if info['cs'] == 'Indexed':
                self._out('/ColorSpace [/Indexed /DeviceRGB ' + str(len(info['pal']) / 3 - 1) + ' ' + str(
                    self.n + 1) + ' 0 R]')
            else:
    ...
    def _arc(self, x1, y1, x2, y2, x3, y3):
        h = self.h
        k = self.k
        self._out(sprintf('%.2f %.2f %.2f %.2f %.2f %.2f c ', x1 * k, (h - y1) * k, x2 * k, (h - y2) * k, x3 * k,
                          (h - y3) * k))
...
if __name__ == "__main__":
    ...
    # print a.read()
    #    a.close()
    # pdf.image(a, type='png', x=10, y=0)
    # pdf.image('b.png', x=0, y=0)
    ...
    a = StringIO()
    a.write(open('start.png', 'rb').read())
    # pdf.image(a, type='png', x=10, y=0)
    # pdf.image('b.png', x=0, y=0)
    pdf.image('c.gif', x=0, y=0)
